# E2E tests — WidgetEmail

Playwright suite for `WidgetEmail`. Config in `../../playwright.config.ts`. Playwright is resolved from the `/GitHub` workspace root (no local `@playwright/test` devDep).
...
- Plugin `WidgetEmail` activated in FS.
